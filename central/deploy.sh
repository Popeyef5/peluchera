...
( sudo crontab -l 2>/dev/null; \
  echo "0 3 * * * docker run --rm -v $CERTBOT_ETC_VOL:/etc/letsencrypt -v $CERTBOT_WWW_VOL:/var/www/certbot certbot/certbot renew --webroot -w /var/www/certbot --quiet && docker compose exec proxy nginx -s reload" \
) | sudo crontab -
